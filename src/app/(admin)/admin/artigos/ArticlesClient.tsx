    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        {articles.length === 0 ? (
          <div className="p-12 text-center space-y-3">
            ...
            <div className="text-sm font-semibold text-slate-600">Nenhum artigo encontrado</div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100/80">
                {articles.map((art) => (
                  <tr key={art.id || art.slug} className="hover:bg-slate-100/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-lg bg-slate-100 border border-slate-200/60 overflow-hidden shrink-0">
                          {art.cover_image ? (
                            <img
                              src={art.cover_image}
                              alt={art.title}
                        ...
                        <div className="min-w-0">
                          <div className="font-bold text-slate-900 truncate max-w-xs">{art.title}</div>
                    ...
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-purple-50 text-purple-600 border border-slate-200/60">
                        {art.category}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="font-medium text-slate-700">{art.author_name}</div>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          href={`/guias/${art.slug}`}
                          target="_blank"
                          className="p-1.5 text-slate-400 hover:text-slate-500 rounded-lg hover:bg-slate-100"
      ...
      {editingArticle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-3xl w-full p-6 space-y-4 shadow-2xl max-h-[92vh] overflow-y-auto">
            ...
            <div className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="font-semibold text-slate-600 block mb-1">Título do Artigo *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="font-semibold text-slate-600 block mb-1">Slug da URL amigável</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="font-semibold text-slate-600 block mb-1">Resumo / Subtítulo</label>
                <textarea
                  rows={2}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="font-semibold text-slate-600 block mb-1">Tempo de Leitura</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="font-semibold text-slate-600 block mb-1">URL da Imagem de Capa</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-semibold text-slate-600 block mb-1">Conteúdo Completo (Markdown / Texto Rico) *</label>
                <textarea
                  rows={10}
                  value={editingArticle.content || ''}
                  onChange={(e) => setEditingArticle({ ...editingArticle, content: e.target.value })}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-900 font-mono leading-relaxed"
                  placeholder="Escreva o artigo completo com parágrafos, subtítulos (##) e tópicos..."
                />
              </div>

              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                <div className="font-semibold text-slate-700">SEO & Indexação</div>
